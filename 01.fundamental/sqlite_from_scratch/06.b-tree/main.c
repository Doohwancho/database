...
- array에 저장하다가 b-tree로 저장하는걸로 바꿈: read의 시간복잡도가 O(N) -> O(log N)으로 더 빨라짐 
- 어느정도 채우다가 꽉찼다? -> split함 (새로운 node + leaf node 만들고 거기에 원래 있던 데이터 나눠 저장함)
- split으로 쪼갤 때 균형을 유지함. 그래야 read 할 때 O(log N)가 일정하게 나옴 
- full scan(leaf node끼리는 linkedlist로 연결되있어서 binary search 계속 안해도 되고 바로 가기 때문에 빠름)
...
typedef struct {
  Table* table;
  uint32_t page_num; //page_num & cell_num 이 두개로 데이터의 인덱스를 나타냄 
  uint32_t cell_num; //page_num & cell_num 이 두개로 데이터의 인덱스를 나타냄 
