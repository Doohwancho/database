...
table에 array<Row> 로 저장함. 
몇 바이트 위치에 저장할지는 row_slot() 함수에서 결정해줌 
*/
...
//new 
// 행 번호 주면 정확한 페이지랑 오프셋 계산해서 찾아줌
// 페이지 없으면 그때 할당함 ㅇㅇ (lazy allocation)
...
PrepareResult prepare_statement(InputBuffer* input_buffer, Statement* statement) {
    if(strncmp(input_buffer->buffer, "insert", 6) == 0) {
        ...
        );                                                                      //inputbuffer에 있던 string을 insert %d %s %s 파싱해서 Statement 객체에 넣기 
        if (args_assigned < 3) {
    ...
    if(strcmp(input_buffer->buffer, "select") == 0) {
        statement->type = STATEMENT_SELECT; //select flag로 바꾼다 
        return PREPARE_SUCCESS;
    }
